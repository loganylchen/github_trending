...
./development.log \
-r ${GOURCE_FPS} \
-o - | ffmpeg -y -r ${GOURCE_FPS} -f image2pipe -probesize 100M -i - \
-y -r ${GOURCE_FPS} -f image2pipe -probesize 100M -i ./tmp/overlay.pipe -i ${mp3} -shortest -map 2:a:0 \
${LOGO} \
...
# ffmpeg  -i ./video/tmp.mp4 -stream_loop -1 -i ${mp3} -shortest -map 0:v:0 -map 1:a:0 -y ./video/output.mp4
...
cp /visualization/video/output.mp4 /visualization/output/
